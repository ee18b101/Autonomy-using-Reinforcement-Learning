...
from tensorflow.keras.layers import Dense, GlobalAveragePooling2D, Conv2D, Activation, AveragePooling2D, Flatten
...
MODEL_NAME = "model_base_64x3_CNN"
EXISITING_MODEL = './models/model_base_64x3_CNN_____4.00max_-105.80avg_-252.00min__1588054723.model' # False fi you want to start newly
...
class DQNAgent:
	def __init__(self):
		...
		if not EXISITING_MODEL:
			self.tensorboard = ModifiedTensorBoard(log_dir=f"logs/{MODEL_NAME}-{int(time.time())}")
		else:
			self.tensorboard = ModifiedTensorBoard(log_dir=f'logs/model_base_64x3_CNN-1588055128')
			print(f'Logging onto model: {EXISITING_MODEL[9:]}')
	...
	def create_model(self):
		if not EXISITING_MODEL:
			...
			model = load_model(EXISITING_MODEL)
			print(f'Loaded Model: {EXISITING_MODEL}')
			return model
